...
Since we were specifically calibrating the trip distribution, its not surprising that on the whole this has improved.  However, overall, mode share estimates appear to have somehow worsed.

## Trip distribution by purpose (improved)

Trip distribution post-calibration now more closely matches VISTA for all modes, except for walking.  This is because trip distances were previously under estimated; now they have increased and are approximately matched.  However, walking trips are the exception: these were previously over-estimated, and now this is slightly more the case; although, the differences are small.  I think with regards to trip distribution, we can consider calibration successful:

## Mode share (worsened)

Although we didn't directly calibrate mode choice, by shifting trips to longer distances this indirectly influences mode choice; for the worse, it seems, despite closer match to VISTA survey data.

In short, fewer people choose to walk, and more people choose to use PT or cycle; car mode share seems similar.

## Weekly trip distance and time estimates per person

- Estimates of person-distance and time spent using different modes were improved post-calibration  for driving/passenger car modes (although still under-estimated)
- Public transport distance was over-estimated, but time was under-estimated (and an improvement on pre-calibration)
- walking distance estimates were improved, but time was over-estimated (make sense perhaps; we over-estimate the length of walking trips)
- cycling distance and time were over-estimated (slightly under-estimated pre-calibration)
