...
if (file.exists(pre_calib_file)) {
  trips_pre <- pre_calib_file %>% fread()
  cat("Loaded pre-calibration trips file: ", pre_calib_file, "\n")
  cat("Records:", nrow(trips_pre), "\n")
} else {
  cat("Pre-calibration file not found. Please select manually.\n")
  trips_pre <- file.choose() %>% fread()
  cat("Loaded pre-calibration trips file manually\n")
}
```
...
if (file.exists(post_calib_file)) {
  trips_post <- post_calib_file %>% fread()
  cat("Loaded post-calibration trips file: ", post_calib_file, "\n")
  cat("Records:", nrow(trips_post), "\n")
} else {
  cat("Post-calibration file not found. Please select manually.\n")
  trips_post <- file.choose() %>% fread()
  cat("Loaded post-calibration trips file manually\n")
}
```
...
trips <- bind_rows(trips_pre, trips_post, trips_scenario_vista, trips_scenario_vista_noWeight) %>%
  mutate(mode = factor(mode, levels = c("Driving Car", "Car Passenger", "Public Transport", 
...
The calibration process shows measurable impacts on trip generation and distribution patterns:

1. **Mode Share**: Compare the mode share improvements between pre- and post-calibration scenarios relative to VISTA reference data.

2. **Trip Length Distributions**: The density plots show how calibration affected the distribution of trip lengths across different transport modes.

3. **Median Distances**: The median distance analysis provides a clear metric for evaluating calibration success, showing which modes moved closer to or further from VISTA observations.

4. **Trip Purposes**: Changes in trip purpose distribution indicate how calibration affected different types of travel behavior.

5. **Weekly Travel Patterns**: Average weekly distance and time per person show the overall impact on travel demand by mode.

### Recommendations for Further Analysis

1. **Detailed Mode Analysis**: Focus on modes that worsened during calibration to understand the trade-offs made.

2. **Spatial Analysis**: Consider spatial distribution of improvements/deteriorations across different areas of Melbourne.

3. **Time-of-Day Analysis**: Examine whether calibration impacts varied by time of day or day of week.

4. **Distance Band Analysis**: Look at specific distance brackets where improvements were most/least effective.
