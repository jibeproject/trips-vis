...
# CARL - please can you replace the two paths below with the path/file names for the full VISTA data
...
# fields for weights [CARL - please can you check that the SA3 field names are correct.  Eg I'm not sure about capitalisation.  I want to use the SA3 weights; the 'else' option of using LGA weights is only there so it will run when I'm using the version that doesn't contain those weights.]
...
trips_vista <- trips_vista %>%
  filter(origSA4_name %in% c("Melbourne - Inner", "Melbourne - Inner East",
           ...
           destSA4_name %in% c("Melbourne - Inner", "Melbourne - Inner East",
                             "Melbourne - Inner South", "Melbourne - North East",
                             "Melbourne - North West", "Melbourne - Outer East",
                             "Melbourne - South East", "Melbourne - West",
                             "Mornington Peninsula"))
...
trips_vista <- trips_vista %>% 
  ...
  mutate(
    mode = case_when(
      ...
      # Mobility Scooter, Motorcycle, Other, and Taxi - CARL - please could you confirm and update
      # this note in the full VISTA version]
      TRUE                                         ~ "Other"
    ),
    ...
    # [SP note: check structure of weights in full VISTA version.  Lines
    ...
    # CARL - please check that weights in the full VISTA version also follow 
    # this pattern
...
# CARL - at this point, please check that 'trips' has the correct fields and data structure as follows
# > str(trips)
...
pop_vista = sum(as.numeric(gsub(",", "", pp_vista[[wdperswgt]])), na.rm = T) +
  sum(as.numeric(gsub(",", "", pp_vista[[weperswgt]])), na.rm = T)  # [CARL - if the weight fields were correct in the  full VISTA as above, then this should also work here]
...
# CARL - everything else from here on should now run as written, and produce
